...
Write a query that retrieves the order details for customers who have placed an order with a TotalAmount greater than $400. Use a subquery to find the customer IDs first, and then retrieve the orders from the Orders table.
  subquery
...
Write a query that retrieves the OrderID, CustomerID, and TotalAmount for each order. Additionally, return the number of orders each customer has placed using a subquery in the SELECT clause.
...
FROM ORDERS
  ...
  SELECT OrderID, CustomerID, TotalAmount,
    (SELECT COUNT(*) 
     FROM Orders  
     WHERE CustomerID = CustomerID) AS total_orders
FROM Orders ;
